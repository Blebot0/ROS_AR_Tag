...
def failsafe(init_dist, init_angle):
	move(init_dist)
	align(-1*init_angle)
	stop()
...
def tag_move(dist, move_dist, large_ar_heading):
	...
	while tag > (dist-move_dist):
		if tag<1:
			failsafe(initial_tag, large_ar_heading)
			return 1
			break
		twist.linear.x=0.8 
		pub.publish(twist)
...
def gate(sign, dist, angle_diff, large, small, large_ar_heading, small_ar_heading):
	...
	failed = tag_move(large, move_dist, large_ar_heading)
	if failed:
		return 
...
def listener():
	...
	rospy.Subscriber("/RSscan", LaserScan, laser)
